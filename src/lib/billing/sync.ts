...
/** Maps a storeAppListingSubscriptionKey → plan name. */
function determinePlan(subKey: string | null | undefined): 'pro' | 'launch_offer' | null {
  if (!subKey) return null;
  const proKey = process.env.IKAS_PRO_SUBSCRIPTION_KEY;
  const launchKey = process.env.IKAS_LAUNCH_OFFER_SUBSCRIPTION_KEY;
  if (proKey && subKey === proKey) return 'pro';
  if (launchKey && subKey === launchKey) return 'launch_offer';
  return 'pro'; // default: treat unknown paid key as pro
}

/** Non-blocking lazy sync — call fire-and-forget on authenticated requests when stale. */
...
/** Full sync: fetches getMerchantLicence and updates merchant_billing. */
export async function syncMerchantBilling(merchantId: string, authorizedAppId: string): Promise<void> {
  try {
    ...
    if (!billing) return; // Will be created by OAuth callback

    if (!currentSub) {
      // Subscription removed — expire if not on trial
      if (billing.plan !== 'trial' && billing.status !== 'expired') {
        await supabaseAdmin
          .from('merchant_billing')
          .update({ status: 'expired', ikas_status: 'REMOVED', ikas_last_synced_at: new Date().toISOString(), updated_at: new Date().toISOString() })
          .eq('merchant_id', merchantId);

        await supabaseAdmin
          .from('billing_events')
          .insert({ merchant_id: merchantId, event: 'cancelled', data: { reason: 'ikas_subscription_removed' } });
      } else {
        await supabaseAdmin
          .from('merchant_billing')
          .update({ ikas_status: 'REMOVED', ikas_last_synced_at: new Date().toISOString(), updated_at: new Date().toISOString() })
          .eq('merchant_id', merchantId);
      }
      return;
    }
    ...
    // Detect renewal: last payment date is newer than stored period start
    const lastPaymentDate = currentSub.lastPaymentDate ? new Date(currentSub.lastPaymentDate as unknown as string) : null;
    const storedPeriodStart = billing.current_period_start ? new Date(billing.current_period_start) : null;
    const isRenewal = lastPaymentDate && storedPeriodStart && lastPaymentDate > storedPeriodStart;
...
/** Create an initial trial billing record for a new merchant install. */
